...
function renderQues() {
  ...
  if (!question[index]) {
    ...
    if (result <= 5) {
      Swal.fire({
        title: `Failed`,
        text: `${userName.value}! Your score is ${result}`,
        icon: "error",
        confirmButtonText: "Okay",
        confirmButtonColor: "#ff4b2b",
      });
    ...
    container.innerHTML = `
        ...
        <div class="optionDiv p-2 m-1"><label for="opt1"><input type="radio" name="option" value="${
          question[index].opt1
        }" onclick="showBtn()" id="opt1" class="form-check-input">  ${
      question[index].opt1
    }</label></div>
        <div class="optionDiv p-2 m-1"><label for="opt2"><input type="radio" name="option" value="${
          question[index].opt2
        }" onclick="showBtn()" id="opt2" class="form-check-input">  ${
      question[index].opt2
    }</label></div>
        <div class="optionDiv p-2 m-1"><label for="opt3"><input type="radio" name="option" value="${
          question[index].opt3
        }" onclick="showBtn()" id="opt3" class="form-check-input">  ${
      question[index].opt3
    }</label></div>
        <div class="optionDiv p-2 m-1"><label for="opt4"><input type="radio" name="option" value="${
          question[index].opt4
        }" onclick="showBtn()" id="opt4" class="form-check-input">  ${
      question[index].opt4
    }</label></div><br>
        <div class="btnDiv">
        <button id="backBtn" class="btn quizbtn hiddenbtn"  onclick="backButton">Back</button>
      ...
      document.getElementById("backBtn").addEventListener("click", function () {
        ...
        container.innerHTML = `
          ...
          question[index - 1].question
            ...
            <div class="optionDiv p-2 m-1"><label for="opt1"><input type="radio" name="option" value="${
              question[index - 1].opt1
            }" onclick="showBtn()" id="opt1" class="form-check-input">  ${
          question[index - 1].opt1
        }</label></div>
            <div class="optionDiv p-2 m-1"><label for="opt2"><input type="radio" name="option" value="${
              question[index - 1].opt2
            }" onclick="showBtn()" id="opt2" class="form-check-input">  ${
          question[index - 1].opt2
        }</label></div>
            <div class="optionDiv p-2 m-1"><label for="opt3"><input type="radio" name="option" value="${
              question[index - 1].opt3
            }" onclick="showBtn()" id="opt3" class="form-check-input">  ${
          question[index - 1].opt3
        }</label></div>
            <div class="optionDiv p-2 m-1"><label for="opt4"><input type="radio" name="option" value="${
              question[index - 1].opt4
            }" onclick="showBtn()" id="opt4" class="form-check-input">  ${
          question[index - 1].opt4
        }</label></div><br>
            <div class="btnDiv">
            <button id="backBtn" class="btn quizbtn" onclick="backButton">Back</button>
            <button onclick="renderQues(); showPerBtn()" id="hiddenbtn" class="btn quizbtn" style="display: none;">Next</button>
